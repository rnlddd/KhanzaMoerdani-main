...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[J]ICD 10","[J]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[M]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[M]Penilaian Awal Keperawatan Ranap Bayi/Anak","[A]Booking MCU Perusahaan","[M]Penilaian Awal Medis Ralan Gigi","[M]Catatan Observasi Restrain Nonfarmakologi"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                if(Sequel.mengedittf("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "penilaian_awal_medis_ralan_gigi='"+tbUser.getValueAt(i,1063).toString()+"',"+
                    "catatan_observasi_restrain_nonfarma='"+tbUser.getValueAt(i,1064).toString()+"'")==true){
                    emptTeks();
                }
            }         
        }
                            ...
                            try {
                                ...
                                if(i!= -1){
                                    Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                        ...
                                        "penilaian_awal_medis_ralan_gigi='"+tbUser.getValueAt(barisdicopy,1063).toString()+"',"+
                                        "catatan_observasi_restrain_nonfarma='"+tbUser.getValueAt(barisdicopy,1064).toString()+"'");
                                }
                                userdicopy="";
                                copyhakakses="";
                                barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.skrining_kesehatan_gigi_mulut_remaja,user.penilaian_awal_keperawatan_ranap_bayi,user.booking_mcu_perusahaan,user.penilaian_awal_medis_ralan_gigi,user.catatan_observasi_restrain_nonfarma from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
                    ...
                    try {
                        if(rs.getString(1).toLowerCase().contains(TCari.getText().toLowerCase())||
                            ...
                            tabMode.addRow(new Object[]{rs.getString(1),
                               ...
                               rs.getBoolean("penilaian_awal_medis_ralan_gigi"),
