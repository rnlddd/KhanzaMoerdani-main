...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            penilaian_awal_medis_ralan_gigi=false,catatan_observasi_restrain_nonfarma=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.skrining_kesehatan_gigi_mulut_remaja,user.penilaian_awal_keperawatan_ranap_bayi,user.booking_mcu_perusahaan,user.penilaian_awal_medis_ralan_gigi,"
              + "user.catatan_observasi_restrain_nonfarma from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    penilaian_awal_medis_ralan_gigi=rs.getBoolean("penilaian_awal_medis_ralan_gigi");catatan_observasi_restrain_nonfarma=rs.getBoolean("catatan_observasi_restrain_nonfarma");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[M]Penilaian Awal Medis Ralan Gigi".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[M]Penilaian Awal Medis Ralan Gigi",penilaian_awal_medis_ralan_gigi});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[M]Penilaian Awal Medis Ralan Kandungan".equals(tbUser.getValueAt(i,1).toString())){
              ...
              if("[M]Penilaian Awal Medis Ralan Gigi".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","penilaian_awal_medis_ralan_gigi='"+tbUser.getValueAt(i,2).toString()+"'");
            }
