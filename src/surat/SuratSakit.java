...
public final class SuratSakit extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalAkhir.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-11-2024" }));
        ...
        TanggalAwal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-11-2024" }));
        ...
        FormInput.add(KdDok);
        KdDok.setBounds(100, 70, 140, 23);

        NamaDokter.setEditable(false);
        NamaDokter.setHighlighter(null);
        NamaDokter.setName("NamaDokter"); // NOI18N
        FormInput.add(NamaDokter);
        NamaDokter.setBounds(240, 70, 480, 23);
        ...
        BtnDokter.setBounds(720, 70, 28, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoSurat.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("suratsakit","?,?,?,?,?,?","No.Surat Sakit",6,new String[]{
                    NoSurat.getText(),TNoRw.getText(),Valid.SetTgl(TanggalAwal.getSelectedItem()+""),Valid.SetTgl(TanggalAkhir.getSelectedItem()+""),LamaSakit.getText(),KdDok.getText()
                })==true){
                tabMode.addRow(new String[]{
                    NoSurat.getText(),TNoRw.getText(),TNoRM.getText(),TPasien.getText(),Valid.SetTgl(TanggalAwal.getSelectedItem()+""),Valid.SetTgl(TanggalAkhir.getSelectedItem()+""),LamaSakit.getText(),KdDok.getText(),NamaDokter.getText()
                });
    ...
    private void MnCetakSuratSakitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratSakitActionPerformed
       if(TPasien.getText().trim().equals("")){
            ...
            /*    if(Sequel.cariInteger("select count(kamar_inap.no_rawat) from kamar_inap where kamar_inap.no_rawat=?",TNoRw.getText())>0){
                ...
                kodedokter=Sequel.cariIsi("select suratsakit.kd_dokter from suratsakit where suratsakit.no_surat=?",NoSurat.getText());
                namadokter=Sequel.cariIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?",kodedokter);
                finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",kodedokter);
                    ...
                    param.put("namadokter",namadokter);
        ...
        try{
            ...
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                     ...
                     "from suratsakit inner join reg_periksa on suratsakit.no_rawat=reg_periksa.no_rawat "+
                     "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join dokter on suratsakit.kd_dokter=dokter.kd_dokter "+
